...
cd "$WORKING_DIR"
running_cmd="python $DLJC/dljc -t inference_debug --annotationClassPath "$JSR308"/universe/bin/src --checker GUTI.GUTIChecker --solver GUTI.GUTIConstraintSolver --solverArgs=\"collectStatistic=true,useGraph=false\" -m INFER -afud $WORKING_DIR/annotated -o logs -- $build_cmd "
